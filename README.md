...
| 期权曲线 | `bkcurvfqq` | 每日 **10:00** 的 ATM、25D RR、10D BF、25D BF；完整期限的中间/买/卖隐波 |
...
5. 首次补历史时，进入 `Actions → Backfill China Money FX history → Run workflow`，默认回填 2023-01-01 起的人民币外汇参考汇率、10:00 期权曲线、掉期曲线和美元隐含利率曲线。回填工作流只需运行一次；日常任务仍只补当天。
...
- 期权的四类曲面通过页面已提供的查询控件读取，**固定读取 10:00**，保留当日每个曲面的完整期限表和买/中/卖三档报价。
